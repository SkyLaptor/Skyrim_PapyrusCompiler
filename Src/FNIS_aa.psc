...
bool Function SetAnimGroup(actor ac, string animGroup, int base, int number, string mod, bool debugOutput = false) global
	if ( base >= 0 ) && ( number >= 0 ) && ( number <= 9 ) && ( ac != none )
		...
		elseif !ac.is3dloaded()
			if debugOutput
				Debug.Trace("FNIS aa SetAnimGroup mod: " + mod + " actor: " + (ac.GetBaseObject() as ActorBase).Getname() + " not loaded.")
			endif
 			return false
		endif
		if debugOutput
			Debug.Trace("FNIS aa SetAnimGroup mod: " + mod + " actor: " + (ac.GetBaseObject() as ActorBase).Getname() + " group: " + animGroup + " base: " + base + " number: " + number)
		endif
		...
		;if ac.GetAnimationVariableInt("FNISaa" + animGroup) != ( value ) 
		;	return false
		;endif
	...
	Debug.Trace("FNIS aa SetAnimGroup BAD PARAMETER(S) mod: " + mod + " actor: " + (ac.GetBaseObject() as ActorBase).Getname() + " group: " + animGroup + " base: " + base + " number: " + number)
	return false
endFunction
